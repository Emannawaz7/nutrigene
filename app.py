...
def generate_pdf_report(list_of_data_dicts, list_of_chart_buffers):
    ...
    output = BytesIO()
    pdf.output(output, 'F') # 'F' means "file" and it expects a file-like object if no name is given
    # OR, if you want to be explicit about writing to the buffer:
    # pdf_content = pdf.output(dest='S').encode('latin-1') # Or 'B' for bytes directly
